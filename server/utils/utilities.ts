...
export async function postGame(formData: FormData, db: DrizzleD1Database<_>, session: any, game: any, request: URL ,mode?: "update" ){
  ...
  const parsed = InsertGames.extend({ 
    cover: z.file().max(1024 * 1024).optional(), 
  ...
  const { cover, pictures, categories, files, file_types, file_names, ...gameData } = parsed
  if (cover) await hubBlob().put(`${session.user.id}/${parsed.title}/pictures/${cover.name}`, cover)

  let result;
  if (!mode){
    result = await createGame(gameData,`${prepared_url}/${parsed.title}/pictures/${cover?.name}`,db);
  }
  else{
    result = await editGame(gameData,`${prepared_url}/${parsed.title}/pictures/${cover?.name}`,session,db,game.id)
  }
  ...
  for (const image of pictures ?? []) {
    await hubBlob().put(`${session.user.nickname}/${parsed.title}/pictures/${image.name}`, image)
    await db.insert(tables.pictures).values({ picture_url: `${prepared_url}/${parsed.title}/pictures/${image.name}`, game_id: result.insertedId })
  }
  ...
  for (const [index,value] of files?.entries() ?? []) {
    ...
    await hubBlob().put(`${session.user.nickname}/${parsed.title}/files/${value.name}`, value)
    await db.insert(tables.game_files).values({ type: type! , name: name!, link: `${prepared_url}/${parsed.title}/files/${value.name}`, game_id: result.insertedId })
  }

  return result;
}

async function createGame(gameData: any, cover: string, db: DrizzleD1Database<_>){
  const [res] = await db.insert(tables.games).values({
    ...gameData,
    cover
...
async function editGame(gameData: any, cover: string, db: DrizzleD1Database<_>, session: any, gameid: string){
  const [res] = await db.update(tables.games).set({
    ...gameData,
    cover
  }).where(and(eq(tables.games.id,gameid),eq(tables.games.user_id,session.user.id))).returning({ insertedId: tables.games.id });

  return res;
}

export async function deleteGameData(gameid: string, db: DrizzleD1Database<_>){
  await db.delete(tables.pictures)
    .where(eq(tables.pictures.game_id, gameid))

  await db.delete(tables.game_categories)
    .where(eq(tables.game_categories.game_id, ))

  await db.delete(tables.game_files)
    .where(eq(tables.game_files.game_id, gameid))
  

}

export async function deleteGamePunctuations(gameid: string, db: DrizzleD1Database<_>){
    await db.delete(tables.puntuation)
